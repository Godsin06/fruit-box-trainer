...
class StrategyPy:
    def __init__(self):
        ...
        self.score = 0.0       # 우리가 정의한 '커스텀 점수' 누적 합
...
def build_prefix_sum(grid):
    """
    prefix[r][c] = (0,0)부터 (r-1,c-1)까지 모든 cell의 합
    (크기는 (NUM_ROWS+1) x (NUM_COLS+1)로, 인덱스 편의성을 위해 확장)
    """
...
def rect_sum(prefix, x, y, w, h):
    """
    prefix를 이용해 (y,x)부터 (y+h-1, x+w-1)까지의 합을 O(1)에 구함
    """
    r1, c1 = y, x
    r2, c2 = y+h, x+w
...
def find_strategy_recursive(grid):
    ...
    def recurse(g, current, num_moves):
        ...
        # (중요) 2D Prefix Sum 사용: 사각형 합(=10) 판별
                ...
                for h_ in range(1, NUM_ROWS - y + 1):
                    ...
                    for w_ in range(1, NUM_COLS - x + 1):
                        ...
                        if sub_sum == 10:
                            ...
                            # 더 키울수록 합이 커지므로 break
                            break

        # "rect_score" 큰 순 => 상위 D개
