...
class FruitBoxGUI:
    def __init__(self, root):
        ...
        self.lbl_sum_title = ttk.Label(self.frm_info, text="인식된 사과 합", font=("", 12, "bold"))
    ...
    def play_game(self):
        ...
        if best_score < 150:
            print("[INFO] 예상 점수가 150점 미만이므로 즉시 재실행 (F9)")
            self.root.after(1000, self.on_f9)  # 🎯 1초 후 F9 실행
        ...
        # 6) 모든 드래그 작업 완료 후 점수 체크
        print(f"[INFO] 최종 예상 점수= {best_score}")

        if best_score >= 150:
            print("[INFO] 점수가 150점 이상이므로 Alt + F10 입력")
            pyautogui.hotkey('alt', 'f10')  # 🎯 Alt + F10 자동 입력

        # 7) 2분(120초) 대기 후 재실행 (F9 누른 것처럼)
        print("[INFO] 2분 후 자동 재실행")
        self.root.after(120000, self.on_f9)  # 🎯 120초 후 F9 입력과 동일한 동작 실행
